    ...
    <ul class="list-disc list-inside space-y-2">
        @foreach ($cotizacions as $cotizacion)
            <li class="font-semibold text-sm dark:text-gray-200 leading-tight mb-3 flex justify-between"
                wire:key="cotizacion--{{ $cotizacion->id }}">
                {{ $cotizacion->pedido_id }}                
                <div>                    
                    @if ($cotizacion->pedido->estado == 'Cotizando')                    
                        @can('Enviar una cotizacion de un pedido al cliente')
                        <x-button wire:click="verPedidoCotizacion({{ $cotizacion->id }})">Ver Detalle de la cotizacion</x-button>
                        @endcan
                    @else
                        <x-button wire:click="">Cotizado</x-button>
                    @endif                                        
                </div>
            </li>
        @endforeach
